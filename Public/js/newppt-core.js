...
window.onload = function(){
    ...
    window.GLOBAL.onPlayerInit = function(player) {
        ...
        window.GLOBAL.dynamicPptActionClick = window.GLOBAL.getUrlParams("dynamicPptActionClick") ? window.GLOBAL.getUrlParams("dynamicPptActionClick") == "true" : true;
        ...
        window.GLOBAL.isControl = window.GLOBAL.getUrlParams("control") ? window.GLOBAL.getUrlParams("control") == "true" : false;
        /**
         * [backEvent 是否返回当前PPT操作的event对象]
         */
        window.GLOBAL.isBackEvent = window.GLOBAL.getUrlParams("isBackEvent") ? window.GLOBAL.getUrlParams("isBackEvent") == "true"  : false;
        ...
        window.GLOBAL.actionHandlerFunction = function (data) {
            try {
                ...
                switch (data.action) {
                    ...
                    break;
                    case PLAYDYNAMICPPT:
                        window.GLOBAL.ServiceNewPptAynamicPPT.playbackController.play(false);
                        var allAudios = window.GLOBAL.saveAudioSrc;
                        if(allAudios && allAudios.length > 0){
                          for (var i = 0; i < allAudios.length; i++) {
                            var audioEle = allAudios[i];
                    ...
                    case USERTRIGGERAUDIO:
                       if( document.getElementById('testAudio') ){
                        ...
                        var nowSlideIndex = ts.slideIndex();
                        var nowStepIndex = ts.stepIndex();
                        window.GLOBAL.ServiceNewPptAynamicPPT.clearOldSlideInfo();
                        window.GLOBAL.ServiceNewPptAynamicPPT.playbackController.gotoTimestamp(nowSlideIndex, nowStepIndex);
                ...
                if (noticeParentIFramePlayVideo) { //由上iframe层来play video
                    ...
                    if ($vd) {
                        ...
                        if ($resouce && $resouce.length > 0) {
                            ...
                            //   window.GLOBAL.saveVideoSrc[noticeParentIFrameData.pptslide] = videoData;
                            // }
                        ...
                        //       var index = noticeParentIFrameData.pptslide;
                        //       if(window.GLOBAL.saveVideoSrc[index]){
                    ...
                    if (this && this.__proto__ && this.__proto__.play && typeof this.__proto__.play === 'function') {
                        ...
                        try {
                          ...
                          //     var ts = window.GLOBAL.ServiceNewPptAynamicPPT.playbackController.clock().timestamp();
                          //     var nowSlideIndex = ts.slideIndex();
                          //     if($resouce && $resouce.length <= 0 && window.GLOBAL.saveVideoSrc[nowSlideIndex]){
                          //         $vb.src = window.GLOBAL.saveVideoSrc[nowSlideIndex].src;
        ...
        //添加事件监听,并发送数据 bxk
        if(window.GLOBAL.isControl && window.GLOBAL.isBackEvent){
            ...
            window.GLOBAL.addEvents(document, 'touchmove', _backEvent);
            window.GLOBAL.addEvents(document, 'touchend', _backEvent);
        }
